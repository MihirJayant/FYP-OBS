...
export default function Jobs() {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Posted By",
      cell: ({ row }) => row.original.poster_name || "N/A",
    },
    {
      accessorKey: "status",
      header: "Status",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const isActive = job.status === "open" || job.status === "awarded" || job.status === "in_progress";
        ...
        return (
          <div className="flex gap-1">
            ...
            {/* Cancel job - only if still active */}
            {isActive && (
              <Button
                variant="ghost"
                size="icon"
            ...
            {/* Close job - only if still active */}
            {isActive && (
              <Button
                variant="ghost"
                size="icon"
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="border rounded-lg p-4">
            ...
            <h3 className="text-2xl font-bold text-blue-600">
              {data.filter((j) => j.status === "in_progress").length}
            </h3>
          </div>
          <div className="border rounded-lg p-4">
